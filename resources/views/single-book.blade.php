@extends('app') @section('heading') {{$book_name->name}} <span class="label" style="color:#6D7878 ;font-weight:400">By {{$book_name->author}} </span> @endsection @section('breadcrumb')
...
@endsection @section('headerbuttons')

<button class="btn btn-danger  " data-toggle="modal" data-target="#deleteM"> Remove </button>
...
<div class="container-fluid">
    ...
    <div id="edit_book" class="row">
        ...
        <div class="col-lg-5" style="margin-bottom:10px">
            ...
            <input class="form-control" id="name" placeholder="Name " type="text" required value="{{$book_name->name}}">
        </div>
        <div class="col-lg-5">
            <h5 for="name" class="col-lg-3 control-label" style="font-weight:300;font-size:110%">Author</h5>
            <input class="form-control" id="author" placeholder="Name " type="text" required value="{{$book_name->author}}"> </div>
        <legend></legend>
        <div class="col-lg-4">
...
if ($b->reserved=="1"||$b->reserved==1){
    ...
    echo "No";
                        ...
                        <td>
                            <button type="button" class="btn btn-success btn-block btn-sm" <?php if ($b->status=="1"||$b->status==1){ echo "disabled"; } ?> >Lend</button>
                        </td>
                        <td> <a href='/library/public/single-copy?id={{$b->id}}' class="btn btn-primary btn-block btn-sm">View</a> </td>
                        <td>
                            <button type="button" class="btn btn-danger btn-block btn-sm">Remove</button>
                        </td>
                    </tr>
...
<script>
    ...
    function DelSingleBook(a) {
        ...
        $.ajax({
            type: "get",
            url: '/library/public/deleteSingleBook',
            data: {
